...
async def getImg(url, session, fileName, log):  # 具体页面每个图片
    async with session.get(url) as resp:
        ...
        pages = html.xpath("//div[@class='page-links']/a/@href")
        pageUrls = []
        #  获取具体图片的分页
        if len(pages) > 0:
            pageOne = pages[0].rsplit("/", 2)[0]
            pageUrls.extend(pages)
            pageUrls.insert(0, pageOne)
        ...
        for pageUrl in pageUrls:
            async with session.get(pageUrl) as resp:
                html2 = etree.HTML(await resp.text())
                tasks = []
                figures = html2.xpath("//figure[@class='wp-block-image size-large']")
                for fig in figures:
...
async def getUrl(text, session, log):
    ...
    # 先排除掉什么都搜不到的情况并返回日志，重置程序
    pageCheck = processOne.select("div[class='page-content']")
    if len(pageCheck) > 0:
        log.put(f"搜索不到查询结果" + "      " + traceback.format_exc())
        return
    processOne = BeautifulSoup(resp.text, "html.parser")
